...
* The 2nd block: '_Download target genome_' will download the target genome specified either by NCBI assembly accession number, load a local file, upload a file to Colab, or download preseted _Daucus carota_ genomes. This block also prints the available replicons/chromosomes and their zero based index (cindex) that will be required later.
...
* The 6th block: "_Add 5' Oligonucleotides and restriction enzyme regcognition sites to the fwd and rv primers_" adds a custom oligonucleotide and/or the selected RE binding region to the 5' end of the primers. If the primers will be used for cloning with RE, the sites should be added so that they are considerd by the Primer3 program.
